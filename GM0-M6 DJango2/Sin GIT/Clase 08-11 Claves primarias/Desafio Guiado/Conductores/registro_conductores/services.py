...
def imprimir_modelos():
    ...
    for c in conductores:
        ...
        if hasattr(c, "direccion"):
            d = c.direccion
            print(
                f"dirección: {d.calle} {d.numero} / {d.comuna} /{d.ciudad} / {d.region}"
            )
...
def crear_conductor(rut, nombre, apellido, fecha_nac):
    if not rut.isdigit() and not isinstance(fecha_nac, date):
        ...
        return
    conductor = Conductor(
        rut=rut, nombre=nombre, apellido=apellido, fecha_nac=fecha_nac
    )
    conductor.save()
    imprimir_modelos()
...
def agregar_un_vehiculo(conductor, patente, marca, modelo, year):
    vehiculo = Vehiculo(
        conductor=conductor, patente=patente, marca=marca, modelo=modelo, year=year
    )
    vehiculo.save()
    imprimir_modelos()
