...
def crear_grupo(nombre, fecha_creacion):
    if not isinstance(fecha_creacion, date):
        print(
            "fecha con formato invalido, por favor, \
...
def agregar_album(grupo, titulo, year):
    album = Album(grupo=grupo, titulo=titulo, year=year)
    album.save()
    return album


def obtiene_artista(nombre, apellido):
    return Artista.objects.filter(nombre=nombre).filter(apellido=apellido).first()


def obtiene_grupo(nombre):
    return Grupo.objects.filter(nombre=nombre).first()
...
def artista_participa_albumes(artista):
    ...
    for g in artista.grupos.all():
        ...
        datos = {"grupo": g.nombre, "albumes": albumes}
